...
| `llm.summarize` / `llm.complete` | Common gateway calls, used first by the `SummaryAgent` prototype for agent-tool coupling and model feasibility checks, optional for the broader loop, replaceable, and never the system of record. |
